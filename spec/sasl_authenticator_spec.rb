...
describe KafkaLegacy::SaslAuthenticator do
  let(:logger) { LOGGER }
  ...
  let(:sasl_authenticator) {
    KafkaLegacy::SaslAuthenticator.new(
  ...
  context "when SASL PLAIN has been configured" do
    ...
    it "raises KafkaLegacy::Error when the username or password is incorrect" do
      auth_options[:sasl_plain_password] = "wrong"

      expect {
        sasl_authenticator.authenticate!(connection)
      }.to raise_error(KafkaLegacy::Error, /SASL PLAIN authentication failed/)
    end
  end
  ...
  context "when SASL SCRAM has been configured" do
    ...
    it "raises KafkaLegacy::Error when the username or password is incorrect" do
      ...
      expect {
        sasl_authenticator.authenticate!(connection)
      }.to raise_error(KafkaLegacy::FailedScramAuthentication)
    end
  end
end
