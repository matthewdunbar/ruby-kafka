...
describe "API Versions API", functional: true do
  ...
  example "checks cluster API support" do
    expect(kafka.supports_api?(KafkaLegacy::Protocol::PRODUCE_API)).to eql(true)
    expect(kafka.supports_api?(KafkaLegacy::Protocol::PRODUCE_API, 0)).to eql(true)
    expect(kafka.supports_api?(KafkaLegacy::Protocol::PRODUCE_API, 100)).to eql(false)
